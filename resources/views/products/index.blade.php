        ...
        <tbody>
            @foreach ($products as $product)
                <tr>
                    <td>{{ $product->name }}</td>
                    <td>{{ $product->price }}</td>
                    <td><input type="color" class="form-control form-control-color" id="pwd" name="color"
                    ...
                    <td>
                        <form action="{{ route('productdelete', $product->id) }}" method="POST">
                            @csrf
                            @method('delete')
                            <button onclick="return confirm('آیا از حذف این مورد اطمینان دارید')" type="submit"
                    ...
                    <td><a href="{{ route('producteditpage', $product->id) }}"><button
                                class="btn btn-outline-warning"><svg xmlns="http://www.w3.org/2000/svg" width="16"
                                    height="16" fill="currentColor" class="bi bi-feather" viewBox="0 0 16 16">
                                    <path
